...
通常利用では、ガード本体のソースを利用プロジェクトへ置く必要はありません。venvも必須ではありません。
...
2. 利用プロジェクトに`.codex/hooks/sensitive_columns.tsv`を作成します。

3. 既存の`.codex/config.toml`へ`PreToolUse` hookを追記します。既存設定や他のhookは上書きしません。
...
Codexは対象MCPツールを呼ぶ直前にガードへSQLを渡します。許可ならMCPを実行し、拒否ならMCPを呼ぶ前に停止します。TSV、hook設定、動作確認は後続の章で詳しく説明します。
...
サンプルの[examples/codex/config.toml](../../examples/codex/config.toml)を、project-localな`.codex/config.toml`へ追記・統合して使います。既存ファイルがある場合は上書きしません。
...
- TSVがプロジェクトの`.codex/hooks/`から読み込まれている
...
- Windowsではwrapperから`mcp-sql-result-guard`を解決できるか確認する

### CLIが見つからない

- `command -v mcp-sql-result-guard`または`Get-Command mcp-sql-result-guard`で確認する
- PythonのScriptsディレクトリがPATHに含まれるか確認する
- hookを実行するCodexプロセスから同じPATHが見えるか確認する
