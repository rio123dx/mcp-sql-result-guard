...
CLIをインストールし、利用するプロジェクトにTSVとCodex hook設定を配置します。

1. Codexから見える`PATH`へCLIをインストールします。hookは`mcp-sql-result-guard`をコマンド名で起動します。
...
2. `.codex/hooks/sensitive_columns.tsv`を作成します。
...
3. `.codex/config.toml`へhook設定を追加します。既存ファイルがある場合は上書きせず、既存の`[features]`へ`hooks = true`を追加して、`[[hooks.PreToolUse]]`グループを追記します。他の設定やhookは残し、`[features]`を重複させないでください。
   ...
   Windowsでは、[サンプルwrapper](../../examples/codex/run-sql-guard.ps1)を`.codex/hooks/run-sql-guard.ps1`として保存します。`command_windows`はこのファイルを呼び出します。

4. `matcher`を実際のMCP SQL実行ツール名へ変更し、Codexでhookをレビューして信頼します。
...
└─ .codex/
   ...
   └─ hooks/
      ...
      └─ run-sql-guard.ps1  # Windows wrapperを使う場合のみ
```

Codexは対象MCPツールを呼ぶ直前にガードへSQLを渡します。許可ならMCPを実行し、拒否ならMCPを呼ぶ前に停止します。

### 4.1 fail-open／fail-closedを選択
...
サンプルの[examples/codex/config.toml](../../examples/codex/config.toml)は、project-localな`.codex/config.toml`へ追加またはマージして使います。既存ファイルを上書きせず、他の設定やhookを残してください。CLIのコマンドはCodexから見える`PATH`に必要です。
...
利用プロジェクトのルートで、許可SQLと拒否SQLを標準入力から確認します。
...
許可SQLは標準出力が空になり、拒否SQLは`permissionDecision: "deny"`を含むJSONを返します。あわせて次を確認します。
...
- `mcp-sql-result-guard`コマンドをterminalから単独実行する
- サブディレクトリからCodexを起動してもgit root基準でpathを解決できるか確認する
- Windowsではwrapperと`Get-Command mcp-sql-result-guard`の結果を確認する
